...
test.describe("Milestone 2.1 preview browser verification", () => {
  ...
  test("automatically runs Refresh All when the next refresh countdown elapses", async ({
    page
  }) => {
    const now = new Date().toISOString();
    const user = {
      id: "user_auto_refresh_countdown",
      email: "auto-refresh-countdown@example.invalid",
      createdAt: now
    };
    const session = {
      user,
      session: {
        token: "session_auto_refresh_countdown",
        expiresAt: new Date(Date.now() + 60 * 60 * 1000).toISOString()
      }
    };
    let syncAllRequests = 0;
    ...
    await page.route("**/v1/**", async (route) => {
      ...
      if (method === "POST" && path === "/v1/connectors/sync-all") {
        ...
        notifications = [
          notificationFixture({
            id: "notification_scheduled_refresh",
            title: "Scheduled Refresh All message",
            summary: "Loaded by the automatic countdown refresh",
            createdAt: syncedAt,
            updatedAt: syncedAt
          })
        ];
